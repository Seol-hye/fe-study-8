...
console.log(str.includes("@")); //문자열에 특정 문자(매개변수)기 포함되어있는가?
...
console.log(str.substring(0, 4)); //특정 위치 문자 잘라내기(뽑아내기)
                //   0 <= index < 4
console.log(str.substring(5, 10)); // 5 <= index < 10
console.log(str); //원본 변형됐는지 확인 -> substring 비파괴적 함수
...
console.log("|" + test + "|"); //원본유지 비파괴적 함수
test = test.trim();
console.log("|" + test + "|");

// 문자 분리 split
let phone = "010-1234-8357";
...
console.log(today.getMonth());          //월    0~11 (1~12월)   10? -> 11월
...
console.log(Math.floor(123.999)); //내림
console.log(Math.floor(-123.999));

console.log(Math.trunc(123.55)); //소수점 제거
console.log(Math.trunc(-123.55)); 

console.log(Math.max(10, 50)); //최대값찿기
console.log(Math.min(10, 50)); //최소값찿기
